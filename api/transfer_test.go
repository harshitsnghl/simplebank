...
import (
	...
	mockdb "github.com/harshitsnghl/simplebank/db/mock"
	db "github.com/harshitsnghl/simplebank/db/sqlc"
	"github.com/harshitsnghl/simplebank/token"
...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		// 	},
		// 	buildStubs: func(store *mockdb.MockStore) {
		// 		dummyAccount := db.Account{}
		// 		store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(dummyAccount, db.ErrRecordNotFound)
		// 		store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(0)
		// 		store.EXPECT().TransferTx(gomock.Any(), gomock.Any()).Times(0)
		// 	},
		// 	checkResponse: func(recorder *httptest.ResponseRecorder) {
